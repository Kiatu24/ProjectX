...
public class Util {
    ...
    public static double getDistance(Sprite s1, Sprite s2) {
    	double d = Math.sqrt((s2.x - s1.x) * (s2.x - s1.x) + (s2.y - s1.y) * (s2.y - s1.y));
    	return d;
    }
    
    public static boolean isCloseTo(int x1, int y1, int x2, int y2, int leeway) {
    	if (x1 >= x2 - leeway && x1 <= x2 + leeway && y1 >= y2 - leeway && y1 <= y2 + leeway) {
    		return true;
    	}
    	else {
    		return false;
